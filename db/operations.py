...
#             )
#             if ev.data:
#                 event_tz = ev.data.get("timezone")
#                 event_country = ev.data.get("country")
#         except Exception:
#             pass

#     # ── STEP 2: AUTO-DETECT TIMEZONE FROM COUNTRY ────────────
    

#     # ── STEP 3: FINAL TIMEZONE DECISION ──────────────────────
#     final_tz = resolve_timezone(
#     tz_name=tz_name,
#     event_tz=event_tz,
...
from db.supabase_client import get_client
from utils.datetime_utils import resolve_timezone, to_utc, validate_time_str
...
def _get_event_context(event_id: int | None) -> tuple[str | None, str | None]:
    if not event_id:
        return None, None
    ...
    try:
        ...
        if ev.data:
            return ev.data.get("timezone"), ev.data.get("country")
    except Exception:
        pass
...
def add_match(
    ...
    Validation  → input sanitised before any DB call.
    Duplicate   → same match_date + team1_id + team2_id is rejected.
    Timezone    → resolved via _get_event_context() + resolve_timezone().
    UTC storage → match_datetime derived via to_utc() and stored as TIMESTAMPTZ.
    Cache       → load_matches cleared on every successful write.
    """
    # ── STEP 0: backend validation ───────────────────────────
    match_name = (match_name or "").strip()
    if not match_name:
        return False, "Match name is required."
    if match_date is None:
        return False, "Match date is required."
    if not isinstance(match_date, date):
        return False, "Invalid match date."
    match_time = (match_time or "00:00").strip() or "00:00"
    if not validate_time_str(match_time):
        return False, "Invalid match time format."

    sb = get_client()

    # ── STEP 1: fetch event context ──────────────────────────
    event_tz, event_country = _get_event_context(event_id)

    # ── STEP 2: duplicate guard (same date + both teams) ─────
    if team1_id and team2_id:
        try:
            dup = (
                sb.table("matches").select("id")
                .eq("match_date", str(match_date))
                .or_(
                    f"and(team1_id.eq.{team1_id},team2_id.eq.{team2_id}),"
                    f"and(team1_id.eq.{team2_id},team2_id.eq.{team1_id})"
                )
                .maybe_single()
                .execute()
            )
    ...
    # ── STEP 4: convert to UTC ────────────────────────────────
    try:
        match_datetime_utc = to_utc(match_date, match_time, final_tz)
    except Exception:
        return False, "Failed to convert match datetime."
...
def bulk_add_matches(rows: list[dict]) -> tuple[int, list[str]]:
    ...
    Each dict may contain:
        Required : match_name, match_date (date object)
        Optional : event_id
                   match_time (str HH:MM, default "00:00")
                   tz_name    (IANA str, resolved via resolve_timezone)
                   team1_id, team2_id, venue, notes

    match_datetime is derived from match_date + match_time + resolved timezone
    via to_utc(). No time is ever silently discarded.
    """
    success, warns = 0, []
    for i, r in enumerate(rows):
        match_date = r.get("match_date")
        if match_date is None:
            ...
            continue
        if not isinstance(match_date, date):
            warns.append(f"Row {i+1}: invalid match_date — skipped.")
            continue
        match_name = (r.get("match_name") or "").strip()
        if not match_name:
            warns.append(f"Row {i+1}: missing match_name — skipped.")
            continue
        ok, msg = add_match(
            event_id   = r.get("event_id"),
            match_name = match_name,
            ...
            notes      = r.get("notes", ""),
            match_time = r.get("match_time", "00:00") or "00:00",
            tz_name = (r.get("tz_name") or r.get("timezone") or "").strip(),
        )
        if ok:
            success += 1
        else:
    ...
    # Explicit final clear — guards against partial-success scenarios
    # where individual add_match() calls cleared mid-loop.
    load_matches.clear()
    return success, warns
...
def add_auction(
    ...
    Validation       → input sanitised before any DB call.
    Timezone         → resolved via _get_event_context() + resolve_timezone(),
                       identical pipeline to add_match().
    UTC storage      → auction_datetime derived via to_utc() → stored as TIMESTAMPTZ.
    Cache            → load_auctions cleared on every successful write.
    franchise_name   → NOT written; column kept in DB for backward compat.
    """
    # ── STEP 0: backend validation ───────────────────────────
    auction_name = (auction_name or "").strip()
    if not auction_name:
        return False, "Auction name is required."
    if auction_date is None:
        return False, "Auction date is required."
    if not isinstance(auction_date, date):
        return False, "Invalid auction date."
    auction_time = (auction_time or "00:00").strip() or "00:00"
    if not validate_time_str(auction_time):
        return False, "Invalid auction time format."
    ...
    final_tz = resolve_timezone(
        ...
        event_tz=event_tz,
        country=event_country,
        venue=location.strip().lower(),
    )

    # ── STEP 3: convert to UTC ────────────────────────────────
    try:
        auction_datetime_utc = to_utc(auction_date, auction_time, final_tz)
    except Exception:
        return False, "Failed to convert auction datetime."
    ...
    for _, r in ma.iterrows():
        ...
        rows.append({
            ...
            "metadata": {
                "event":              ev_name,
                "venue":              r.get("venue", ""),
                "match_datetime_utc": mdt.isoformat() if pd.notna(mdt) else None
            },
        })
    ...
    for _, r in au.iterrows():
        ...
        rows.append({
            ...
            "metadata": {
                "event":                ev_name,
                "location":             r.get("location", ""),
                "auction_datetime_utc": adt.isoformat() if pd.notna(adt) else None,
            },
        })
