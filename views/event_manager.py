...
#                        "start_date", "end_date", "country", "gender","timezone"]].copy()
#         disp["start_date"] = disp["start_date"].dt.date
#         disp["end_date"]   = disp["end_date"].dt.date
#         disp.columns = ["Event", "Type", "Category", "Format", "Start", "End", "Country", "Gender","Timezone"]
...
#         if not row.empty:
#             tz_val = row.iloc[0].get("timezone")
#             country_val = row.iloc[0].get("country")

#             event_tz = resolve_timezone(
#                 tz_name=None,
#                 event_tz=tz_val,
#                 country=country_val,
#                 venue=None,
...
#              # Time + timezone row
#             sel_time, sel_tz = _time_tz_row("match", event_tz)
#         with c2:
...
#         # ── Resolve Event Timezone (CORRECT WAY) ────────
#         event_tz = user_tz
...
#         # UX clarity (important)
#         st.caption(f"Auto timezone based on event/country: {event_tz}")

#         # ── Time + Timezone ─────────────────────────────
#         sel_time, sel_tz = _time_tz_row("auction", event_tz)
