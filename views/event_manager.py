...
#     sel_name = st.selectbox("Link to Event", list(event_options.keys()), key=f"es_{key}")
...
#     if leagues_df.empty:
#         st.warning("No leagues found. Create one in the Leagues tab to link events to a parent league.")
#     else:
#         league_options.update({r["league_name"]: int(r["id"]) for _, r in leagues_df.iterrows()})
...
#     # Replaced checkbox with clean dropdown
#     ev_id, ev_name = _event_search_select("match")
...
#     <div class="page-header">
#         <div><h1>EVENT MANAGER</h1>
#         <p>Tournaments · Leagues · Matches · Registrations · Auctions</p></div>
#     </div>""", unsafe_allow_html=True)
...
#         "Tournament / Series", "Leagues", "Matches", "Registration", "Auctions"
#     ])
#     with tab1: _tab_tournament()
#     with tab2: _tab_leagues()
#     with tab3: _tab_matches()
#     with tab4: _tab_registration()
#     with tab5: _tab_auction()
...
def _tab_tournament() -> None:
    ...
    # Quietly update league options if they exist (no warning message anymore)
    if not leagues_df.empty:
        league_options.update({r["league_name"]: int(r["id"]) for _, r in leagues_df.iterrows()})
...
def _tab_matches() -> None:
    ...
    # Note: Event selection remains OUTSIDE the form so it can dynamically load teams in real-time
    ev_id, ev_name = _event_search_select("match")
...
def _tab_registration() -> None:
    ...
    with st.form("add_reg_form", clear_on_submit=True):
        # Event dropdown successfully moved INSIDE the form
        ev_id, ev_name = _event_search_select("reg")
...
def render() -> None:
    ...
    # Leagues tab smoothly moved to the end
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
